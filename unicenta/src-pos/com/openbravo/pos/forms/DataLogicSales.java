...
*/    
    public final ProductInfoExt getProductInfoByUShortCode(String sCode) throws BasicException {
		...
		+ "FROM products "
                + "WHERE LEFT( CODE, 7 ) = ?"                
//  force selection of 7 digits ie: 2123456 specific to allow for other 12 digit
//  codes that may be in use at positions 234567
//  last digit (position 7) can be used to identify COUPON (5 or 9)                
            , SerializerWriteString.INSTANCE
    ...
    public final SentenceList getTaxCustCategoriesList() {
        return new StaticSentence(s
            ...
            , new SerializerRead() {
                @Override
                public Object readValues(DataRead dr) throws BasicException {
                    return new TaxCustCategoryInfo(
                            dr.getString(1), 
                            dr.getString(2));
            }});
    }
 ...
 public Object readValues(DataRead dr) throws BasicException {
    ...
    // new ticket
        ...
        SentenceExec ticketstatusupdate = new PreparedSentence(s
            ,"UPDATE tickets SET STATUS =" + ticket.getTicketId() + " WHERE TICKETID = " + ticket.getTicketStatus()+ " "
            ,SerializerWriteParams.INSTANCE );
        
        SentenceExec ticketlineinsert = new PreparedSentence(s
            , "INSERT INTO ticketlines (TICKET, LINE, PRODUCT, ATTRIBUTESETINSTANCE_ID, "
                + "UNITS, PRICE, TAXID, ATTRIBUTES) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
	...
	return new SentenceExecTransaction(s) {
		...
		, new SerializerWriteBasicExt(productsRow.getDatas(), 
			...
			if (new PreparedSentence(s
                            ...
                            , new SerializerWriteBasicExt(productsRow.getDatas(), new int[] {30, 0})).exec(params) == 0) {
                            new PreparedSentence(s
				, "INSERT INTO products_cat (PRODUCT, CATORDER) VALUES (?, ?)"
    ...
    public final SentenceExec getStockDiaryInsert1() {
        ...
        public int execInTransaction(Object params) throws BasicException {
            ...
            ? new PreparedSentence(s
                ...
                return new PreparedSentence(s
                    , "INSERT INTO stockdiary (ID, DATENEW, REASON, LOCATION, PRODUCT, "
                            + "ATTRIBUTESETINSTANCE_ID, UNITS, PRICE, AppUser, SUPPLIER, SUPPLIERDOC) "
                            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                    , new SerializerWriteBasicExt(stockdiaryDatas, 
